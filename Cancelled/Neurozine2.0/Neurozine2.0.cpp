// Neurozine2.0.exe - Malware
// I will not finish this buggy malware and make mp3 instead, then my best one yet
...
DWORD WINAPI mouseInvertCircle(LPVOID lpParam)
{
...
DWORD WINAPI blr(LPVOID lpParam) { //credits to N17Pro3426
	HDC hdc;
	int sw = GetSystemMetrics(0), sh = GetSystemMetrics(1), size = 2005;
	while (1) {
		hdc = GetDC(0); HDC hdcMem = CreateCompatibleDC(hdc);
		HBITMAP screenshot = CreateCompatibleBitmap(hdc, sw, sh);
		SelectObject(hdcMem, screenshot);
...
DWORD WINAPI fakeerror(LPVOID lpParam) {
	MessageBoxW(NULL, L"The system has reached the maximum size allowed for the system part of the registry. Additional storage requests will be ignored.", L"Windows - Low On Registry Space", MB_ICONERROR);
	return 0;
}
...
{
	if (MessageBoxW(NULL, L"This is a GDI Only, Run?", L"Neurozine2.0.exe by pankoza and Venra (GDI Only)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? It still contains flashing lights - NOT for epilepsy", L"F?i?n?a?l? ?W?a?r?n?i?n?g? - Neurozine2.0.exe (GDI Only)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		...
		//	HANDLE thread1dot2 = CreateThread(0, 0, blr, 0, 0, 0);
			sound1();
			Sleep(30000);
			...
			HANDLE thread7dot1 = CreateThread(0, 0, mouseInvertCircle, 0, 0, 0);
			sound7();
			Sleep(30000);
